...
export default function TaskPayablePage() {
  ...
  const handleDownloadPDF = () => {
    ...
    // Add BayriKo logo directly as text with styling
    const logoY = footerY - 22; // Position logo above footer text
    
    // Create a stylized text logo instead of using an image
    ...
    const logoX = (docPageWidth - logoTextWidth) / 2;
    
    doc.text(logoText, logoX, logoY);
    
    // Add a small line under the logo text
    const lineWidth = logoTextWidth * 1.2;
    const lineX = (docPageWidth - lineWidth) / 2;
    const lineY = logoY + 2;
    ...
    // Make clickable area match the text logo
    const clickableArea = {
      x: logoX - 2,  // Slightly wider than the text
      y: logoY - 10, // Start above the text
      width: logoTextWidth + 4, // Slightly wider than the text
      height: 14     // Height to cover text and underline
    };
