...
import { Pencil, Trash2, FolderKanban, ChevronDown, ChevronRight } from 'lucide-react';
...
export function TaskTable({ tasks }: TaskTableProps) {
  ...
  const tasksByProject = useMemo(() => {
    const groupedTasks: { [key: string]: { projectId: number | null, projectName: string, tasks: Task[] } } = {};
  ...
  const renderProjectGroup = (projectGroup: { projectId: number | null, projectName: string, tasks: Task[] }, index: number) => {
    return (
      <Accordion type="multiple" defaultValue={['item-0']} className="w-full border-b border-dark-border last:border-b-0 bg-dark-surface/50" key={index}>
        <AccordionItem value={`item-${index}`} className="border-b-0">
          <AccordionTrigger className="px-4 py-3 hover:bg-dark-border/30 no-underline">
            <div className="flex items-center">
              ...
              <span className="text-sm font-medium">{projectGroup.projectName}</span>
          ...
          <AccordionContent className="pt-0 pb-0">
            <div className="overflow-x-auto">
              <table className="min-w-full">
                ...
                <tbody>
                  {projectGroup.tasks.map(renderTaskRow)}
                </tbody>
              </table>
            </div>
          </AccordionContent>
